...
// Delete an Auction (New Feature)
app.delete('/auction/:id', authenticate, async (req, res) => {
  try {
    console.log("Deleting auction with ID:", req.params.id);
    const { id } = req.params;
...
app.put('/auction/:id', authenticate, async (req, res) => {
  try {
    ...
    res.json({ message: 'Auction updated successfully', item: updatedAuction });
  } catch (error) {
